...
const Produit = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get("http://localhost:8087/api/");
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(`${API_URL}/${products[editIndex].id}`, formState);
        ...
        const response = await axios.post(`${API_URL}/add`, formState);
  ...
  const handleDeleteProduct = async (index) => {
    try {
      await axios.delete(`${API_URL}/${products[index].id}`);
