 ...
 const fetchProducts = async () => {
  try {
    ...
    if (Array.isArray(response.data)) {
      ...
      console.error("Les données des produits ne sont pas dans le format attendu");
    }
  } catch (error) {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        const response = await axios.post("http://localhost:8087/api/produits/add", formState);
