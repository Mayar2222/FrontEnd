...
const Produit = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8087/api/produits')
      .then(response => {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      if (isEditing) {
        const updatedProduct = await axios.put(`http://localhost:8087/api/produits/${formState.sku}`, formState);
  ...
  const handleDeleteProduct = async (sku) => {
    try {
      await axios.delete(`http://localhost:8087/api/produits/${sku}`);
