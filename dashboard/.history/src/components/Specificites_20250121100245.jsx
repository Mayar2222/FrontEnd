...
function Specificities() {
    ...
    const handleAddSpecificity = async (e) => {
        ...
        try {
            const response = await axios.post("http://localhost:8087/api/specificites/add", {
                specificityName,
                category: { id: selectedCategory },
                isColor,
            });
            setSpecificities((prev) => [...prev, response.data]); // Mettre à jour l'état local
                    ...
                    {specificities.length > 0 ? (
                        specificities.map((specificity) => (
                            <li key={specificity.id} className="p-2 border-b border-gray-200 flex justify-between items-center">
                                <span>{specificity.name} ({specificity.category?.name || "Non spécifié"})</span>
                            </li>
                        ))
                    ) : (
