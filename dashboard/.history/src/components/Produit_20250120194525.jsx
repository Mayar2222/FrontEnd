...
const Produit = () => {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`http://localhost:8080/api/produits/${products[editIndex].id}`, productData);
  ...
  const handleDeleteProduct = async (index) => {
    try {
      await axios.delete(`http://localhost:8080/api/produits/${products[index].id}`);
