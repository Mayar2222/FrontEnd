...
const Specificites = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isEditing) {
      await axios.put(`http://localhost:8087/api/specificites/${specificites[editIndex].id}`, formState);
      setIsEditing(false);
      setEditIndex(null);
    } else {
      await axios.post('http://localhost:8087/api/specificites/add', formState);
    }
    setFormState({
      specificiteName: '',
      category: '',
  ...
  const handleDelete = async (index) => {
    await axios.delete(`http://localhost:8087/api/specificites/${specificites[index].id}`);
