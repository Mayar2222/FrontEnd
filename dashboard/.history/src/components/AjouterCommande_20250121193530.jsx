...
const Commande = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const commandes = await axios.get('http://localhost:8080/api/commandes');
      setOrders(commandes.data);
      const produits = await axios.get('http://localhost:8080/api/produits');
  ...
  const handleAddOrder = async (e) => {
    ...
    if (isEditing) {
      await axios.put(`http://localhost:8087/api/commandes/${orders[editIndex].id}`, newOrder);
      ...
      const addedOrder = await axios.post('http://localhost:8087/api/commandes/add', newOrder);
  ...
  const handleDeleteOrder = async (index) => {
    await axios.delete(`http://localhost:8087/api/commandes/${orders[index].id}`);
