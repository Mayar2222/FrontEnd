...
const Produit = () => {
  ...
  const handleChange = (e) => {
    ...
    // Si la catégorie change, réinitialisez les spécificités
    if (name === 'category') {
      setFormState(prevState => ({
        ...prevState,
        specificities: {}, // Réinitialisez les spécificités lors du changement de catégorie
      }));
    }
  };
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await axios.put(`http://localhost:8087/api/produits/${products[editIndex].id}`, formData, {
          headers: {
        ...
        response = await axios.post("http://localhost:8087/api/produits/add", formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
                ...
                <div className="space-y-4">
                  ...
                  <label>
                    ...
                    <input
                      type="file"
                      multiple
                      accept="image/*" // Limiter les types de fichiers
                      name="images"
